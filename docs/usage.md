...
- 条件输入：第 `0` 帧和第 `9` 帧
- 恢复目标：中间 `8` 帧
- 方法主线：`Frame Autoencoder + Conditional Latent DiT/Flow Matching`
...
## 4. 训练 Flow Matching / DiT

先把 `configs/flow_matching_base.yaml` 中的 `model.autoencoder.checkpoint` 改成已训练 AE 权重，然后执行：
...
```bash
torchrun --nproc_per_node=4 scripts/train_flow_matching.py --config configs/flow_matching_base.yaml
```

集群脚本方式：

```bash
EXP_NAME=combustion-flow-v0 \
AUTOENCODER_CKPT=results/autoencoder/combustion-ae-v0/checkpoints/best.pt \
WANDB_ENABLED=true \
bash entrypoints/entry_train_flow_matching.sh \
  trainer.val_every_steps=500 \
  data.batch_size=2
```

Flow matching 只对中间 8 帧 latent 建模速度场，首尾帧始终作为固定条件输入。
训练过程中支持按步数插入 validation，并完整采样一条验证样本保存预览图，同时同步到 WandB。
